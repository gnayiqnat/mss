...
export default function SetPassword() {
   ...
   useEffect(() => {
        ...
        if (!userEmail) {
            getEmail().then(r => {r && setUserEmail(r)})
        }
    }, []);
    ...
    async function handleSubmit() {
        ...
        if (newPassword && passwordValidation) {
            ...
            enqueueSnackbar('Password does not fit requirements', { variant: 'error', preventDuplicate: true });
            setIsLoading(false);
        }
    }
    ...
    return (
        <>
            ...
            >
                    <Box
                        ref={scope}
                        sx={{ display: 'flex',width: '100vw', padding: '0px 40px' }}
                    >
                        <Box
                            sx={{
                                display: 'flex',
